...
- In dev mode (`NODE_ENV=development`), the `/admin` route bypasses `ADMIN_USER_IDS` checks — any authenticated user can access the admin dashboard. Production still requires explicit admin IDs.
